...
void setup() {
  ...
  // Silence connectivity probes
  server.on("/connecttest.txt", HTTP_GET, [](AsyncWebServerRequest *request){ request->send(200, "text/plain", "Microsoft NCSI"); });
  ...
  server.on("/command", HTTP_POST, [](AsyncWebServerRequest *request){
    ...
    if (action == "ping") {
        ...
        if (request->hasParam("data", true)) {
            String hexData = request->getParam("data", true)->value();
            // Basic length check - must be even and not empty
            if (hexData.length() > 0 && hexData.length() % 2 == 0) {
                size_t len = hexData.length() / 2;
                uint8_t* buf = new uint8_t[len];
                hexToBytes(hexData, buf);
                broadcastPacket(buf, len);
                delete[] buf;
            }
        }
    }
...
void broadcastPacket(uint8_t* data, size_t length) {
  ...
  Serial.print("Broadcasting (Bits): ");
  for(size_t i = 0; i < length; i++) {
    for(int bit = 7; bit >= 0; bit--) {
      Serial.print((data[i] >> bit) & 0x01);
      if(bit == 4) Serial.print(" "); // Space between nibbles
    }
